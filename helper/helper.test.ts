...
describe('File Utilities', () => {
  describe('validateFileType', () => {
    it('returns extension for valid PDF', async () => {
      const file = createMockFile('document.pdf', 'application/pdf', '%PDF-1.4');
    ...
    it('throws for invalid file type', async () => {
      ...
      await expect(utils.validateFileType(file)).rejects.toThrow(/Invalid file format/);
    });
  });
  ...
  describe('validatePdfUrl', () => {
    it('accepts valid PDF URL', async () => {
      (fetch as any).mockResolvedValue({
        ok: true,
        headers: {
          get: (key: string) => (key === 'content-type' ? 'application/pdf' : '1024'),
        },
        status: 200,
      });
    ...
    it('rejects invalid URL', async () => {
      await expect(utils.validatePdfUrl('not-a-url')).rejects.toThrow(/Invalid URL/);
    });
    ...
    it('rejects non-PDF content', async () => {
      ...
      await expect(utils.validatePdfUrl('https://example.com/file.pdf')).rejects.toThrow(/Link is not a PDF/);
    });
  });
  ...
  describe('validateFileHeader', () => {
    ...
    it('throws for invalid PDF header', async () => {
      ...
      await expect(utils.validateFileHeader(file)).rejects.toThrow(/does not appear to be a valid PDF/);
    });
  });
  ...
  describe('saveFileFromUrl', () => {
    it('fetches and saves PDF file', async () => {
      ...
      const path = await utils.saveFileFromUrl('https://example.com/file.pdf', 'file.pdf');
